...
public class GameField : MonoBehaviour
{
    ...
    private void Awake() {
        for(int i = 0; i < gridSize; i++)
        {
            ...
            {
                grid[i].Add(new Tile(-1, empty, new Side()));
            }
        }   
    ...
    public void DrawNewTile(Tile tile, int x, int y)
    {
        ...
        temp.top = tile.sides.top;
        temp.right = tile.sides.right;
        temp.bottom = tile.sides.bottom;
        temp.left = tile.sides.left;
    ...
    public bool IsPossibleToPlace(Tile tile, int x, int y)
    {
        ...
        for(int i = -1; i <= 1; i++)
        {
            for(int j = -1; j <= 1; j++)
            {
                ...
                if(cur.id == -1)
                    continue;
                if(j == -1 && tile.sides.bottom != cur.sides.top)
                    return false;
                if(j == 1 && tile.sides.top != cur.sides.bottom)
                    return false;
                if(i == -1 && tile.sides.left != cur.sides.right)
                    return false;
                if(i == 1 && tile.sides.right != cur.sides.left)
                    return false;
            }
        }
        return true;
